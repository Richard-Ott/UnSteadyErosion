...
%% Priors -----------------------------------------------------------------
T =  [1,6e3];      % time of step change OR spike in yrs [min,max]
E = [10 1e3];
LOSS = [0,150];     % loss of soil in cm [min,max], can be commented if no spike model
...
forward_model = @(m) Nforward_wrapper(m,sp,consts,scenarios{i},tdata.steps,Nlogical);
...
%% Posterior sampling
tic
[models, logLike] = gwmcmc(mini,{logical_prior logLike},3e8,'ThinChain',5,'burnin',.2);
toc
...
[h1,autocorr] = autocorrelationplot(models)
...
if export
    ...
    exportgraphics(h1,['./output/' filetag '_' scenarios{i} '_autocorrelation.pdf'],'ContentType','vector')
    exportgraphics(h2,['./output/' filetag '_' scenarios{i} '_chains.pdf'],'ContentType','vector')
    exportgraphics(h3,['./output/' filetag '_' scenarios{i} '_cornerplot.pdf'],'ContentType','vector')
    exportgraphics(h4,['./output/' filetag '_' scenarios{i} '_barplot.pdf'],'ContentType','vector')
    exportgraphics(h5,['./output/' filetag '_' scenarios{i} '_datafit.pdf'],'ContentType','vector')
    clear h1 h2 h3 h4 h5
    save(['./output/workspace_' filetag '_' scenarios{i} '.mat'],  '-v7.3')
end
